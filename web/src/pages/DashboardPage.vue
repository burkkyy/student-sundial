<template>
  ...
  <v-btn color="primary" :to="`/dashboardU`">StuView</v-btn>

  <!-- Add New Course Dialog -->
  <v-dialog v-model="newCourseDialog" max-width="600px">
    <v-card>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="error" @click="newCourseDialog = false">Cancel</v-btn>
        <v-btn color="primary" @click="saveNewCourse">Save</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>

  <!-- New Course Event Dialog -->
  <v-dialog v-model="newEventDialog" max-width="600px">
    <v-card>
      <v-card-title>{{ editingEvent ? 'Edit Event' : 'Add New Event' }}</v-card-title>
      <v-card-text>
        <v-form ref="eventForm" @submit.prevent="saveNewEvent">
          <v-text-field
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="error" @click="newEventDialog = false">Cancel</v-btn>
        <v-btn color="primary" @click="saveNewEvent">Save</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
...
const saveNewEvent = async () => {
  ...
  if (editingEvent.value) { // update existing event
